...
1. 前往 Releases 页面下载最新版EXE文件

2. 右键EXE文件 →以管理员身份运行（必须，否则无法正常发送文本）
...
# 打包命令（替换图标路径为你的ico文件绝对路径）
pyinstaller -F -w --icon=C:\Users\你的用户名\Desktop\cs2_icon.ico --noupx cs2_toolbox.py
...
1. 用在线工具（convertio）将图片转为标准ICO

2. 使用绝对路径打包：pyinstaller -F -w --icon=绝对路径\cs2_icon.ico --noupx cs2_toolbox.py

3. 若仍无效，可使用 Resource Hacker 手动替换EXE图标
